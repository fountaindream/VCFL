...
class TestSet(Dataset):
  ...
  def extract_feat(self, normalize_feat):
    ...
    """
    feats, global_feats, local_feats, ids, cams, im_names, marks = \
      [], [], [], [], [], [], []
    done = False
    step = 0
    printed = False
    ...
    while not done:
      ...
      feat, global_feat, local_feat = self.extract_feat_func(ims_)
      feats.append(feat)
      ...
      print('start')
      feat = {'feat':feats,'name':im_names}
      scipy.io.savemat('/media/4TDisk/lfy/ICCV/feat.mat',feat)
      print('done')
  ...
  def eval(
    ...
    # result = {'gallery_f':global_feats[g_inds],'gallery_label':ids[g_inds],'gallery_cam':cams[g_inds],
    #          'query_f':global_feats[q_inds],'query_label':ids[q_inds],'query_cam':cams[q_inds]}
    # scipy.io.savemat('pytorch_result.mat',result)

    # result1 = {'mquery_f':global_feats[mq_inds],'mquery_label':ids[mq_inds],'mquery_cam':cams[mq_inds]}
